            ...
            <div class="box-body p-20">
                <div class="row m-b-15">
                    <div class="col-sm-6">
                        Show
                        &nbsp;&nbsp;
                        <select class="form-control sl-form-control input-sm" v-model="paginationData.per_page" @change="loadGroups(currentPageUrl)">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <table class="table table-bordered table-hover table-striped table-condensed table-paginated">
                            ...
                            <single-group-row v-for="group in groups" :current-group="group" @reloadGroups="loadGroups(currentPageUrl)"></single-group-row>
                            </tbody>
                            <tbody v-else>
                            <tr>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <a href="/user-management/group/create" class="btn btn-primary btn-sm btn-flat">CREATE NEW GROUP</a>
                    </div>
                    <div class="col-sm-6 text-right">
                        <button class="btn btn-default btn-sm btn-flat" @click.prevent="loadGroups" :disabled="paginationData.current_page == 1">FIRST</button>
                        <button class="btn btn-default btn-sm btn-flat" @click.prevent="loadGroups(prevPageUrl)" :disabled="prevPageUrl == null">PREV</button>
                        <button class="btn btn-default btn-sm btn-flat" @click.prevent="loadGroups(nextPageUrl)" :disabled="nextPageUrl == null">NEXT</button>
                        <button class="btn btn-default btn-sm btn-flat" @click.prevent="loadGroups(lastPageUrl)" :disabled="paginationData.current_page == paginationData.last_page">LAST</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    import singleGroupRow from './SingleGroupRow.vue';
    ...
    export default {
        ...
        methods: {
            loadGroups(link){
                ...
                axios.get(link).then(response=> {
                    if (response.data.status == true) {
                        ...
                        this.paginationData.current_page = response.data.groups.current_page;
                        this.paginationData.from = response.data.groups.from;
                        this.paginationData.last_page = response.data.groups.last_page;
                        this.paginationData.next_page_url = response.data.groups.next_page_url;
                        this.paginationData.per_page = response.data.groups.per_page;
                        this.paginationData.prev_page_url = response.data.groups.prev_page_url;
                        this.paginationData.to = response.data.groups.to;
                        this.paginationData.total = response.data.groups.total;
                    }
                }).catch(error=> {
            ...
            setOrdering(column){
                ...
                this.loadGroups(this.currentPageUrl);
            },
            onFilterChanged(){
        ...
        computed: {
            currentPageUrl: function () {
                return '/user-management/group?page=' + this.paginationData.current_page
            ...
            lastPageUrl: function () {
                return '/user-management/group?page=' + this.paginationData.last_page
                        + '&orderBy=' + this.orderByData.column
                        + '&direction=' + this.orderByData.direction
                        + '&per_page=' + this.paginationData.per_page
                        + '&key=' + this.filterText;
