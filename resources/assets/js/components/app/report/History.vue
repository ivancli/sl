    ...
    <div>
        <div class="row m-b-15">
            <div class="col-sm-6">
                Show
                &nbsp;&nbsp;
                <select class="form-control sl-form-control input-sm" v-model="paginationData.per_page" @change="loadHistoricalReports(currentPageUrl)">
        ...
        <div class="row">
            <div class="col-sm-12">
                <table class="table table-bordered table-hover table-striped table-condensed table-paginated">
                    ...
                    <tbody v-if="historicalReports.length > 0">
                    <single-historical-report-row v-for="historicalReport in historicalReports" :current-historical-report="historicalReport" @reload-historical-reports="loadHistoricalReports(currentPageUrl)"></single-historical-report-row>
                    </tbody>
                    <tbody v-else>
                    <tr>
...
<script>
    ...
    import singleHistoricalReportRow from './history/SingleHistoricalReportRow.vue';

    export default{
        components: {
            singleHistoricalReportRow,
        },
        data(){
            return {
                historicalReports: [],
        ...
        methods: {
            loadHistoricalReports(link){
                if (typeof link !== 'string') {
                    link = this.firstPageUrl;
                }
                axios.get(link).then(response => {
                    if (response.data.status === true) {
                        this.historicalReports = response.data.historicalReports.data;
                        this.paginationData.current_page = response.data.historicalReports.current_page;
                        this.paginationData.from = response.data.historicalReports.from;
                        this.paginationData.last_page = response.data.historicalReports.last_page;
                        this.paginationData.next_page_url = response.data.historicalReports.next_page_url;
                        this.paginationData.per_page = response.data.historicalReports.per_page;
                        this.paginationData.prev_page_url = response.data.historicalReports.prev_page_url;
                        this.paginationData.to = response.data.historicalReports.to;
                        this.paginationData.total = response.data.historicalReports.total;
                    }
                }).catch(error => {
            ...
            setOrdering(column){
                ...
                this.loadHistoricalReports(this.currentPageUrl);
            },
            onFilterChanged(){
        ...
        computed: {
            currentPageUrl: function () {
                return '/historical-report?page=' + this.paginationData.current_page
            ...
            firstPageUrl: function () {
                return '/historical-report?page=1&orderBy=' + this.orderByData.column
                    + '&direction=' + this.orderByData.direction
                    + '&per_page=' + this.paginationData.per_page
                    + '&key=' + this.filterText;
            },
            lastPageUrl: function () {
                return '/historical-report?page=' + this.paginationData.last_page
                    + '&orderBy=' + this.orderByData.column
                    + '&direction=' + this.orderByData.direction
                    + '&per_page=' + this.paginationData.per_page
                    + '&key=' + this.filterText;
