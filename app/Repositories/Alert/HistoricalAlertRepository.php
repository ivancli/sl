<?php
/**
 * Created by PhpStorm.
 * User: Ivan
 * Date: 28/05/2017
 * Time: 9:00 PM
 */

namespace App\Repositories\Alert;


use App\Contracts\Repositories\Alert\HistoricalAlertContract;
use App\Models\Alert;
use App\Models\HistoricalAlert;
use Illuminate\Http\Request;

class HistoricalAlertRepository implements HistoricalAlertContract
{
    protected $request;

    public function __construct(Request $request)
    {
        $this->request = $request;
    }

    /**
     * load filtered / all historical alerts
     * @param array $data
     * @return mixed
     */
    public function filterAll(array $data = [])
    {
        $length = array_get($data, 'per_page', 25);
        $orderByColumn = array_get($data, 'orderBy', 'id');
        $orderByDirection = array_get($data, 'direction', 'asc');
        $builder = auth()->user()->historicalAlerts()->with(['alertable']);
        $builder = $builder->orderBy($orderByColumn, $orderByDirection);
        if (array_has($data, 'key') && !empty(array_get($data, 'key'))) {
            $key = array_get($data, 'key');
            $builder->where('id', 'LIKE', "%{$key}%");
            $builder->orWhere('alert_type', 'LIKE', "%{$key}%");
            $builder->orWhere('comp_type', 'LIKE', "%{$key}%");
            $builder->orWhere('comp_price', 'LIKE', "%{$key}%");
            $builder->orWhere('comp_operator', 'LIKE', "%{$key}%");
            $builder->orWhere('created_at', 'LIKE', "%{$key}%");
        }
        $historicalAlerts = $builder->paginate($length);
        if ($historicalAlerts->count() == 0) {
            $page = 1;
            $this->request->merge(compact(['page']));
            $historicalAlerts = $builder->paginate($length);
        }
        return $historicalAlerts;
    }

    /**
     * create historical alert
     * @param Alert $alert
     * @param array $data
     * @return mixed
     * @internal param $email
     */
    public function store(Alert $alert, array $data)
    {
        $user = $alert->user;
        $alertable = $alert->alertable;

        $historicalAlert = HistoricalAlert::create([
            'alert_type' => $alert->alert_type,
            'comp_type' => $alert->comp_type,
            'comp_price' => $alert->comp_price,
            'comp_operator' => $alert->comp_operator,
            'email' => array_get($data, 'email', null)
        ]);

        $user->historicalAlerts()->save($historicalAlert);
        if (!is_null($alertable)) {
            $alertable->historicalAlerts()->save($historicalAlert);
        }
        
        return $historicalAlert;
    }
}